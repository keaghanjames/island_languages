...
fml <- formula('WE ~ weighted_sums + res')
...
# fml <- formula('WE ~ dist11k + dist25k + dist_continents + area_km2 + w_mean_growing_season + 
#    max_elevation  + w_mean_temperature_seasonality + w_mean_Seasonality_Precipitation + 
#    abs_lat + weighted_sums + res')

fml <- formula('WE ~  dist25k + dist_continents + area_km2 + w_mean_growing_season + 
   max_elevation  + w_mean_temperature_seasonality + w_mean_Seasonality_Precipitation + 
   weighted_sums + res')
...
for(i in 1:length(model_combos)) {
  model_combos[[i]] <- paste0('WE ~ ', paste0(c(model_combos[[i]]), collapse = ' + ' ),'+ weighted_sums + res')
...
model_combos <- c(model_combos, formula(WE ~ weighted_sums + res))

require(pbmcapply)
BICs <- pbmclapply(model_combos, autoglm_handler, mc.cores = 3)
save(BICs, file = 'BIC_WE.rdata')
save(model_combos, file = 'model_combosWE.Rdata')
